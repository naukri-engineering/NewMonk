...
(function(w) {
    ...
    boomr = {
        ...
        /** Function Modified for callback property*/
        ...
        /** Function Modified for beacon_callback*/
        sendBeacon: function() {
            ...
            /*DNS Lookup time, Added at Naukri, 4/5/2013 */
            var p = w.performance || w.msPerformance || w.webkitPerformance || w.mozPerformance;
            if (p && p.timing) {
                var pt = p.timing;
                if (pt)
                    BOOMR.addVar('nt_dns', pt.domainLookupEnd - pt.domainLookupStart);
            }
            ...
            /** Delete extra variable from beacon, At Naukri, 5/4/2013*/
            BOOMR.removeVar('rt.bstart', 'rt.end', 'rt.tstart', 'v')
                ...
                for (k in impl.vars) {
                    if (impl.vars.hasOwnProperty(k)) {
                        ...
                        url[encodeURIComponent(k)] = (
                            impl.vars[k] === undefined || impl.vars[k] === null ? '' : typeof impl.vars[k] != 'object' ? encodeURIComponent(impl.vars[k]) : impl.vars[k]
                        )
                    }
                }
            } else {
    ...
    BOOMR.plugins.RT = {
        ...
        // load when the page is usable by the user
            ...
            if (impl.responseStart) {
                ...
                impl.timers.hasOwnProperty('impl.t_fb_approx')
...
var nLogger = {
        init: function(params) {
            ...
            /** Common Values */
            this.orig = encodeURIComponent(params.origin || document.location.href);

            // if (!nLogger.isWhiteListed(this.orig))
            //     return;
            ...
            if ((verOffset = this.nAgt.indexOf("Opera")) != -1) {
                ...
                this.browserVer = this.nAgt.substring(verOffset + 6);
                if ((verOffset = this.nAgt.indexOf("Version")) != -1)
                    this.browserVer = this.nAgt.substring(verOffset + 8);
            }
            // In MSIE, the true version is after "MSIE" in userAgent
            else if (this.nAgt.match(/(?:MSIE |Trident\/.*; rv:)(\d+)/)) {
                this.browserName = "Microsoft IE";
                var ver=this.nAgt.match(/(?:MSIE |Trident\/.*; rv:)(\d+)/);
                this.browserVer =parseInt(ver[1]);
            }
            // In Chrome, the true version is after "Chrome" 
            else if ((verOffset = this.nAgt.indexOf("Chrome")) != -1) {
                this.browserName = "Chrome";
                var verString=this.nAgt.substring(verOffset + 7);
                this.browserVer = verString.match(/\d+.\d+.\d+/)[0];
            }
            // In Safari, the true version is after "Safari" or after "Version" 
            else if ((verOffset = this.nAgt.indexOf("Safari")) != -1) {
                this.browserName = "Safari";
                this.browserVer = this.nAgt.substring(verOffset + 7);
                if ((verOffset = this.nAgt.indexOf("Version")) != -1)
                    this.browserVer = this.nAgt.substring(verOffset + 8);
            }
            ...
            this.ip = params.userIP;
            this.tag = params.tag || '';

            /** nLogger Config */
            var plugin = params.nLogger;
            if (plugin) {
                this.eReport = true;
                this.errorURL = plugin.beaconUrl;
                this.maxErs = plugin.maxErrors || 10;
                this.errCount = 1;
                this.errorDetect();
            }

            /** Boomerang Config*/
            var plugin = params.boomerang;
            if (plugin) {
                this.bReport = true;
                this.boomURL = plugin.beaconUrl;
                this.imageURL = plugin.imageURL;
                this.logBW = (typeof plugin.logBW == 'undefined' || plugin.logBW) ? true : false;
                this.boomInit(this.boomURL, this.imageURL, this.ip, this.logBW, this.appId);
            }

            /** HeatMap Config*/
            var plugin = params.heatmap;
            if (plugin) {
                this.heatMapInit(plugin);
            }

            /** TNM Config*/
            var plugin = params.tnm;
            if (plugin) {
                this.tnmInit(plugin);
            }
        ...
        errorDetect: function() {
            window.onerror = function(msg, url, line) {
                nLogger.logError('',msg, url, line, null)
            }
        },
        heatMapReportData: function(hpData) {
            for (var i = 0; i < hpData.length; i++) {
                nLogger.sendBeacon(hpData[i] + "&rad=" + Math.random(), function(i) {
                    return function() {
                        hpData.splice(i, 1);
                    }
                }(i));
        ...
        heatMapInit: function(plugin) {
            ...
            try {
                var hpData = JSON.parse(localStorage.getItem('hpData'));
                if (!hpData)
                    hpData = []; // Create Empty Array
                else
                    nLogger.heatMapReportData(hpData); //Report allready logged Urls
            } catch (e) {}
            ...
            this.on({
                ...
                name: 'load',
                func: function() {
                    nLogger.getScript(plugin.pluginBaseUrl + 'heatmap.min.js', function() {
                        if (typeof HEATMAP != 'undefined') {
                            HEATMAP.init();
                            try {
                                window.setInterval(function() {
                                    try {
                                        if ((data.coord = HEATMAP.get_coord()).length) {
                                            hpData.push(plugin.beaconUrl + "?data=" + nLogger.stringify(data)); //Temporary Strore
                                            nLogger.heatMapReportData(hpData); //Report data                                                                            
                                        }
                                    } catch (e) {}
                                }, plugin.interval || 5000)
            ...
            this.on({
                ...
                func: function() {
                    try {
                        if ((data.coord = HEATMAP.get_coord()).length) { // If coords available
                            hpData.push(plugin.beaconUrl + "?data=" + nLogger.stringify(data)); //Store
                        }
                        if (hpData) //If Logs available
                            localStorage.setItem('hpData', JSON.stringify(hpData)); // Store backup in localStorage         
                    } catch (e) {}
        ...
        /**
         * [sendStored_AnalyticsData : send stored data form localStorage to server]
         * @param  {[type]}   APIArray [This API array contain objects in each index and object having key & value where
         *                                    key: It's a API-data key name, corresponding data is stored
         *                                    value: It's contain URL where data need to be send.]
         * @param  {Function} callback [a callback function]
         */
        sendStored_AnalyticsData: function(APIArray, callback) {
            var data;
            for (var x in APIArray) {
                for (var y in APIArray[x]) {
                    data = localStorage.getItem(y);
                    if (data) {
                        nLogger.sendBeacon(APIArray[x][y] + "?" + "data" + "=" + data, function(key) {
                            return function() {
                                callback(key);
                            }
                        }(y));
        ...
        tnmInit: function(plugin) {
            var _this = this;
            nLogger.getScript(plugin.pluginBaseUrl + 'tnmplugin.js', function() {
                if (typeof tnm != 'undefined') {
                    // create constructor of tnm object
                    var inst = new tnm();
                    var formNode;
                    // initialize tnm call
                    inst.init(_this.tag, window.location.href);
                    for (var x in plugin.formIds) {
                        formNode = document.getElementById(plugin.formIds[x]);
                        if (formNode) {
                            // bind winlow.unload event to send TNM data to server
                            _this.on({
                                elem: window,
                                name: 'unload',
                                func: function() {
                                    inst.LogSubmit.call(inst, plugin.formIds[x]);
                                }
                            })
                        }
                    }
            ...
            this.sendStored_AnalyticsData([{
                "tnmData": plugin.beaconUrl
            }], function(key) {
                localStorage.removeItem(key);
            });
        },

        /** If url is a whitelisted url return true, else return false; */
        isWhiteListed: function(url) {
            return true;
        },
        boomInit: function(bURL, iURL, ip, logBW, appId) {
            logBW = false;
            BOOMR.init({
                beacon_callback: function(param) {
                    param.appId = appId;
                    param.tag = nLogger.tag;
                    if (param["rt.start"] != "none") {
                        nLogger.reportData(param, bURL)
                    }
                },
                user_ip: ip,
                BW: {
        ...
        logError: function(excpName,msg, url, line, cId) {
            var x, errorList, os, z, errorLogs;
            if(excpName==''){
                excpName='Unknown';
            }
            ...
            var bListed = {
                file: ['\\.logs\\.naukri\\.com\/'],
                message: ["Script error\\."]
            };

            if (typeof msg !== "undefined" && this.errCount <= this.maxErs) {
                this.errCount++;
                errorLogs = {
                    source: "browser",
                    appId: this.appId,
                    ...
                    exceptions: [{
                        ...
                        type: excpName,
                        message: msg,
                        file: !url ? null : encodeURIComponent(url),
                        line: !line ? null : line,
                        catchIdentifier: !cId ? null : cId,
                        screenResolution: this.screenRes,
                        origin: this.orig,
                        tag: this.tag
                    }]
                };
            ...
            wListed.file_flag = nLogger.isWhiteListed(errorLogs.exceptions[0]['file']);

            for (var b in bListed) {
                var arr = bListed[b];
                for (var key in arr) {

                    var temp;
                    temp = (b == 'file') ? decodeURIComponent(errorLogs.exceptions[0][b]) : errorLogs.exceptions[0][b];

                    if ((errorLogs.exceptions[0][b] || '').match(arr[key]))
                        bListed[b + '_flag'] = true;
                }
            }

            if (wListed.file_flag && !bListed.file_flag && !bListed.message_flag)
                this.reportErrData(errorLogs, this.errorURL);
            this.errors = [];
        }
    },
    throwErr: function(excp, cId) {
        if (this.eReport) {
            this.logError(excp.name, excp.message, null, null, cId)
        }
    },
    reportErrData: function(dta, url) {
        if (dta && url) {
            nLogger.sendBeacon(url + "?errorLogs=" + this.stringify(dta) + "&rad=" + Math.random());
        }
    },
    reportData: function(dta, url) {
        if (dta && url) {
            nLogger.sendBeacon(url + "?data=" + this.stringify(dta) + "&rad=" + Math.random());
        }
    },
            ...
            JSON.stringify = function(o) {
                ...
                for (key in o) {
                    if (o.hasOwnProperty(key))
                        (typeof o[key] != "object") ? str += '\"' + key + '\":\"' + o[key] + '\",' : str += '\"' + key + '\":' + JSON.stringify(o[key]) + ',';
                }
                str = str.replace(/,$/, '');
                return '{' + str + '}';
            }
    ...
    on: function(obj) { // Cross borwser on for adding event
        if (window.addEventListener) {
            obj.elem.addEventListener(obj.name, function(event) {
                obj.func.call(this, event);
            })
        } else {
            obj.elem.attachEvent('on' + obj.name, function() {
                obj.func.call(window.event.srcElement, window.event);
            })
        }
    },
    /**
            isBeacon : Boolean, decide whether to delete script tag after response
         */
    getScript: function(scriptUrl, callback, isBeacon) {

        var a = document.createElement('script');
        var _callback = function() {
            callback();
            if (a.parentNode)
                isBeacon ? a.parentNode.removeChild(a) : '';
        }
    ...
    sendBeacon: function(scriptUrl, callback) {
        this.getScript(scriptUrl, callback || function() {}, true);
    }
}
